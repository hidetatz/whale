...
func main() {
	...
	for _, transA := range []blas.Transpose{blas.Trans, blas.NoTrans} {
		for _, transB := range []blas.Transpose{blas.Trans, blas.NoTrans} {
			fmt.Printf("=== %v x %v\n", transA, transB)
			...
			for size := 16; size <= 2048; size *= 2 {
				...
				err := blas.Sgemm(transA, transB, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc)
