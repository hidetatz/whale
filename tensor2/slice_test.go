...
func TestIndex(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name:     "vector 3",
			tensor:   Vector([]float64{1, 2, 3, 4, 5}),
			args:   []*Slice{FromTo(2, 4)},
			expected: Vector([]float64{3, 4}),
		},
		{
			name:     "vector 4",
			tensor:   Vector([]float64{1, 2, 3, 4, 5}),
			args:   []*Slice{FromToBy(1, 5, 2)},
		...
		},
		{
			name:     "vector 8",
			tensor:   Vector([]float64{1, 2, 3, 4, 5}),
			args:   []*Slice{FromTo(-2, 8)},
			expected: Vector([]float64{1, 2, 3, 4, 5}),
		},
		{
			name:     "vector 9",
			tensor:   Vector([]float64{1, 2, 3, 4, 5}),
			args:   []*Slice{FromTo(6, 10)},
			expected: Vector([]float64{}),
		},
		{
			name:     "vector 10",
			tensor:   Vector([]float64{1, 2, 3, 4, 5}),
			args:   []*Slice{FromTo(6, 1)},
			expected: Vector([]float64{}),
		},
		{
			name:     "vector 11",
		...
		},
		{
			...
			args:   []*Slice{FromToBy(0, 4, 2), FromToBy(1, 4, 2)},
			expected: MustNdShape([]float64{2, 4, 10, 12}, 2, 2),
		},
		{
			name:     "2d 2",
			tensor:   MustNdShape(seq[float64](1, 25), 6, 4),
			args:   []*Slice{All(), FromToBy(1, 4, 2)},
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			got, err := tc.tensor.Slice(tc.args...)
